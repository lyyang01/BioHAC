...
PROMPT_CODE = """You're a retrieve augmented coding assistant. You answer user's questions or make modifications based on your own knowledge and the
...
class RetrieveUserProxyAgent(UserProxyAgent):
    ...
    def retrieve_docs(self, problem: str, n_results: int = 20, search_string: str = ""):
        ...
        '''
        problem_list = problem.lower().split()
        doc_list = []
        for i in self._docs_path:
        ...
        for idx, i in enumerate(doc_list):
            for j in i:
                if j in problem_list:
                    _docs_path = [self._docs_path[idx]]
                    indicator = 1
                    break
        '''
        ...
        if indicator == 0:
            _docs_path = ['./tools/empty.md']
        
        
        name = _docs_path[0].split('/')[-1].split('.')[0]
            
        #if name in self._problem_list:
        ...
        if not self._collection or not self._get_or_create:
            ...
            try:
              self._client = create_vector_db_from_dir(
                  ...
                  client=self._client,
                  collection_name=name,#self._collection_name,
              ...
              self._problem_list.append(name)
              #self._collection = True
              #self._get_or_create = True
              self._collection = True
              self._get_or_create = True
