...
speaker_message_list = defaultdict(list)
    ...
    def run_chat(
        ...
        for i in range(groupchat.max_round):
            ...
            try:
                ...
                if speaker.name == 'Checker':
                    #self._groupchat.messages[-1]['content'] is the prompt into Checker.
                    cot_prompt = "According to the execution results, please think step by step. First, "
                    #import pdb
                    #pdb.set_trace()
                ...
                if speaker.name == 'Checker':
                    ...
                    #    pdb.set_trace()
                    #
                    if message['content'] in speaker_message_list[speaker.name]:
                        add_terminate = True
                    #
                    ...
                    for ii in speaker_message_list[speaker.name]:
                        #import pdb
                        #pdb.set_trace()
                        #temp_ii = ii.split('\n')
                        #try:
                        ...
                        try:
                            ...
                            pass
                        temp_ii = temp_ii.split(" ")    
                            #if temp[-5:-1] == temp_ii[-5:-1]:
                        if temp[0:9] == temp_ii[0:9]:
                            add_terminate=True
                            break
                        #except:
                        #    import pdb
                        #    pdb.set_trace()
                    #
                    if ("Great job" in reply) or ("great job" in reply) or add_terminate:
                        reply = reply + "\n\n" + "TERMINATE"
                
                #if speaker.name == 'Corrector':
                #    if ("Great Job!" in reply):
                #        reply = reply + "\n\n" + "TERMINATE"
                #if reply is not None:
                #    if speaker.name == 'code_proxy' and ('exitcode: 1' in reply):
                #        reply = reply.split('[\'\\x1b[0;31m')[0]

                if speaker.name == 'code_proxy2':
                    if speaker._human_input[-1] == 'psudo-exit':
                        for agent in groupchat.agents:
                            if agent.name == 'code_proxy':
                                speaker = agent
                                break
                        reply = speaker.generate_reply(sender=self)
                    if len(speaker._human_input) > 0:
                        if speaker._human_input[-1] == 'retrieve':
                            #import pdb
                            #pdb.set_trace()
                            try:
                                tmp = reply['content']
                ...
                if speaker.name == 'code_proxy':
                    #import pdb
                    #pdb.set_trace()
                    #ansi_escape = re.compile(r'\\x1B(?:[@-Z\\-_]|\[[0-?]*[ -/]*[@-~])')
                    #ansi_escape.sub('', reply)
                    if reply is not None:
                        try:
                ...
                #    for agent in groupchat.agents:
                #if speaker.name == 'Planner':
                #    user_proxy_sender = self
                #if speaker.name == 'toolselect_caller':
                #    import pdb
                #    pdb.set_trace()



                speaker_message_list[speaker.name].append(message['content'])
